...
COM_PORT_DEFAULT   = 'COM6';       % Default COM port
BAUDRATE           = 19200;         % Serial baud rate
TIMEOUT_S          = 30;            % Serial timeout in seconds
...
CMD_REVERSE        = 'Start_Reverse';  % Reverse command
...
% Start buttons
hStart = uicontrol(fig,'Style','pushbutton','String','Start Forward',...
    'Position',[20 100 120 30],'Callback',@onStart,'Enable','off');

hStartRev = uicontrol(fig,'Style','pushbutton','String','Start Reverse',...
    'Position',[280 100 120 30],'Callback',@onStartReverse,'Enable','off');
...
% Move Kick controls a bit lower to avoid overlap
uicontrol(fig,'Style','text','String','Kick count','HorizontalAlignment','left',...
    'Position',[290 78 80 18]);
hKick = uicontrol(fig,'Style','edit','String',num2str(KICK_COUNT),...
    'Position',[370 75 60 24],'Enable','off');

hLog = uicontrol(fig,'Style','listbox','Position',[20 20 480 50],'Max',2,'Min',0);
logMsg('App ready');
...
%% ===== CALLBACKS =====
    ...
    function doConnect()
        ...
        try
            ...
            set([hDisconnect hSpeed hSpeedEdit hForward hReverse hStart hStartRev hStop hKick],'Enable','on');
    ...
    function onDisconnect(~,~)
        ...
        set([hDisconnect hSpeed hSpeedEdit hForward hReverse hStart hStartRev hStop hKick],'Enable','off');
    ...
    function onStart(~,~)
        % Start forward: send speed then forward with optional kick repeats
        if ~state.connected
            logMsg('Not connected');
            return
        end
        set([hStart hStartRev hStop hDisconnect],'Enable','off');
        c = onCleanup(@() set([hStart hStartRev hStop hDisconnect],'Enable','on')); %#ok<NASGU>
        try
    ...
    function onStartReverse(~,~)
        % Start reverse: send speed then a single reverse command
        if ~state.connected
            logMsg('Not connected');
            return
        end
        ...
        try
            ...
            % Reverse once
            logMsg(sprintf('Start reverse at %d', spd));
            sendLine(state.s, [CMD_REVERSE NEWLINE]);
        catch ME
            logMsg(['Reverse error: ' ME.message]);
        end
    end
